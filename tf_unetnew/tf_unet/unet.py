...
def create_conv_net(x, keep_prob, channels, n_class, layers=3, features_root=16,
    ...
    logging.info("Layers {layers}, features {features},"
                 " filter size {filter_size}x{filter_size}, pool size:"
...
class Unet(object):
    ...
    def __init__(self, channels=3, n_class=2, cost="cross_entropy", cost_kwargs={}, **kwargs):
        ...
        self.jaccard_coef_int_cal = self.jaccard_coef_int(self.y, self.predicter)

    def jaccard_coef(self, y_true, y_pred):
        smooth = 1e-12
        intersection = tf.reduce_sum(y_true * y_pred, [0, -1, -2])
        sum_ = tf.reduce_sum(y_true + y_pred, [0, -1, -2])
...
class Trainer(object):
    ...
    def __init__(self, net, verification_batch_size=1, batch_size=1, norm_grads=False, optimizer="momentum", opt_kwargs={}):
        self.net = net
