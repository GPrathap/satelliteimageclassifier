...
def np_to_tfrecords(X, Y, file_path_prefix, verbose=True):
    ...
    def _dtype_feature(ndarray):
        ...
        if dtype_ == np.float64 or dtype_ == np.float32:
            return lambda array: tf.train.Feature(float_list=tf.train.FloatList(value=ndarray))
        elif dtype_ == np.int64:
            return lambda array: tf.train.Feature(int64_list=tf.train.Int64List(value=ndarray))
        else:
...
xx = np.random.rand(3, 3, 3)
yy = np.random.rand(3, 3, 1)
